...
| --- | --- | --- |
| Spoofing | A request is sent without the configured identity, or credentials reach a host the operator did not configure. | Every client attaches credentials through the crate-private `AuthRuntime` (`auth.rs`). Base and token URLs must be HTTPS, or HTTP on a loopback host, with no embedded credentials, query, or fragment (`endpoint.rs`; broker, wallet, and tokenization clients; `AuthRuntime::build`). Issuer request paths resolve against the configured origin and a foreign or scheme-relative target is rejected before authentication (`endpoint::resolve_path`). The issuer, broker, wallet, tokenization, and token-mint HTTP clients do not follow redirects. `KmsJwtAuth` and `AuthRuntime` cannot be built with arbitrary URLs or HTTP clients from outside the crate. |
...
| Denial of service | A malformed value panics, or a retry or poll never ends. | No `unwrap` or `expect` outside tests (Clippy denies them). Retries are limited to classified transient errors: the issuer policy retries at most 5 times with jittered backoff; wallet polling retries 5xx at most 10 times. Every poll is bounded: conversion orders (300 s, then cancel and a 30 s settle window), wallet transfers (30 min), tokenization requests (`PollingConfig` timeout), account activities (1000 pages and a repeated-token check). Rate limits surface as `Backpressure` with the `Retry-After` hint so the consumer can back off. |
...
- Credential tests: insecure base and token URLs rejected for every client,
  foreign request targets rejected, redirects not followed.
