...
use crate::broker::mock::{AlpacaBrokerMock, TEST_ACCOUNT_ID};
...
sol! {
    /// The ERC-20 surface the mint executor calls on the mock's token
    /// contracts; any deployed ERC-20 (e.g. liquidity's `DeployableERC20`)
    /// satisfies it.
    #[sol(rpc)]
    interface IERC20 {
...
/// (the ITN wire name of Base).
const MOCK_NETWORK: &str = "base";
                    ...
                    if let Some(req) = guard.requests.get_mut(idx) {
                        ...
                        if receipt.status() {
                            ...
                            if let Ok(symbol) = Symbol::new(&req.underlying_symbol)
                                && let Ok(delta) = Float::from_raw(B256::ZERO) - quantity
    ...
    server.mock(|when, then| {
        ...
        then.respond_with(move |request: &HttpMockRequest| {
            let MintRequest {
                ...
                idempotency_key,
                network,
            } = match parse_mint_request(request) {
                Ok(parsed) => parsed,
...
/// A validated mint request: the body fields plus the optional
...
/// Validates a mint body, answering 400 on the first missing or invalid
/// field. `client_request_id` defaults to a fresh UUID when absent.
fn parse_mint_request(request: &HttpMockRequest) -> Result<MintRequest, HttpMockResponse> {
                                ...
                                if req.request_type == TokenizationRequestType::Mint {
                                    ...
                                    req.status = match redemption_outcome {
                                        RedemptionOutcome::Complete => {
                                            // Shares return to the broker
                                            // (unlocked from tokenization backing).
                                            if let Ok(symbol) = Symbol::new(&req.underlying_symbol)
                                                && let Err(error) =
