...
impl AlpacaTokenizationService {
    ...
    pub fn new(
        ...
        let client = AlpacaTokenizationClient::new(base_url, account_id, auth, chain)?;
    ...
    /// # Errors
    ///
    /// See [`AlpacaTokenizationError`]; [`AlpacaTokenizationError::is_definitive_mint_rejection`]
    /// tells a proven rejection apart from an uncertain submission outcome.
    ...
    /// against the provider's `client_request_id`.
    ///
    /// # Errors
    ///
    /// Returns `DuplicateMintIssuerRequestId` if the history holds more than
    /// one matching mint, or the API/parse error.
    ...
    /// # Errors
    ///
    /// Returns `RequestNotFound`, `WrongNetwork`, `NetworkMissing`, or the
    /// API/parse error.
    ...
    /// # Errors
    ///
    /// Returns `WrongNetwork`, `NetworkMissing`, or the API/parse error.
    pub async fn find_redemption_by_tx(
        &self,
        tx_hash: &TxHash,
    ) -> Result<Option<TokenizationRequest>, AlpacaTokenizationError> {
    ...
    /// List all pending tokenization requests.
    ///
...
impl AlpacaTokenizationClient {
    ...
    async fn find_mint_by_issuer_request_id(
        ...
        find_unique_mint_request(&body, issuer_request_id).inspect_err(|error| {
            error!(
                target: "tokenization",
                body_len = body.len(),
                error = %error,
                %issuer_request_id,
                "Failed to reconcile mint from tokenization request history"
            );
        })
    }
...
fn find_unique_mint_request(
    body: &str,
    issuer_request_id: &IssuerRequestId,
) -> Result<Option<TokenizationRequest>, AlpacaTokenizationError> {
    let requests: Vec<Value> = serde_json::from_str(body)?;
    let client_request_id = ClientRequestId::from(issuer_request_id);
    let mut matching = requests.iter().filter(|request| {
        request.get("type").and_then(Value::as_str) == Some("mint")
            && request.get("client_request_id").and_then(Value::as_str)
                == Some(client_request_id.as_ref())
    });
    ...
    Ok(Some(serde_json::from_value(request.clone())?))
}
...
mod tests {
    ...
    use super::*;
    use crate::tokenization::{issuer_request_id, tokenization_request_id};

    const TEST_ACCOUNT_ID: AlpacaAccountId =
        AlpacaAccountId::new(uuid!("904837e3-3b76-47ec-b432-046db621571b"));

    /// Every network the client can be bound to, standing in for
    /// liquidity's `Chain::ALL`.
    const ALL_CHAINS: [Chain; 5] = [
        Chain::Base,
        Chain::Ethereum,
        Chain::HyperEvm,
        Chain::Robinhood,
    ...
    fn create_test_client(server: &MockServer) -> AlpacaTokenizationClient {
        create_test_client_on(server, Chain::Base)
    }

    #[tokio::test]
    async fn mint_redirect_does_not_forward_credentials() {
        let source = MockServer::start_async().await;
        let destination = MockServer::start_async().await;
        let client = create_test_client(&source);
        let redirected_request = destination.mock(|when, then| {
            when.any_request();
            then.status(200);
        });
        ...
        let result = client.request_mint(create_mint_request()).await;

        redirect.assert();
        redirected_request.assert_calls(0);
        assert!(
            matches!(result, Err(AlpacaTokenizationError::ApiError { status, .. })
            if status == StatusCode::TEMPORARY_REDIRECT)
        );
    }

    #[test]
    fn client_rejects_credentialed_cleartext_remote_url() {
        let invalid_result = AlpacaTokenizationClient::new(
            "not a URL".to_string(),
            TEST_ACCOUNT_ID,
            test_auth(),
            Chain::Base,
        ...
        let result = AlpacaTokenizationClient::new(
            "http://alpaca.example.com".to_string(),
            TEST_ACCOUNT_ID,
            test_auth(),
            Chain::Base,
        );
    ...
    fn create_test_service_from_mock(server: &MockServer) -> AlpacaTokenizationService {
        create_test_service(create_test_client(server))
    }
    ...
    fn create_test_client_on(server: &MockServer, chain: Chain) -> AlpacaTokenizationClient {
        AlpacaTokenizationClient::new(server.base_url(), TEST_ACCOUNT_ID, test_auth(), chain)
    ...
    async fn mint_request_carries_the_chain_wire_name_for_every_chain() {
        ...
        for chain in ALL_CHAINS {
            let client = create_test_client_on(&server, chain);
    ...
    async fn mint_response_on_another_network_is_refused() {
        let server = MockServer::start();
        let client = create_test_client_on(&server, Chain::Base);
    ...
    #[tokio::test]
    async fn test_request_mint_unclassified_bad_request_remains_uncertain() {
        let server = MockServer::start();
        let client = create_test_client(&server);
    ...
    #[tokio::test]
    async fn find_mint_by_issuer_request_id_scans_request_history() {
        let server = MockServer::start();
        let client = create_test_client(&server);
    ...
    #[tokio::test]
    async fn find_mint_by_issuer_request_id_returns_none_when_not_found() {
        let server = MockServer::start();
        let client = create_test_client(&server);
        let issuer_id = issuer_request_id("missing-mint");
        let issuer_id_string = issuer_id.to_string();
        let lookup_mock = server.mock(|when, then| {
            when.method(GET)
                .path(tokenization_requests_path())
            ...
            then.status(200)
                .header("content-type", "application/json")
                .json_body(json!([{
                    "tokenization_request_id": "redeem_1",
                    "type": "redeem",
                    "status": "completed",
                    "underlying_symbol": "AAPL",
                    "qty": "1",
                    "client_request_id": issuer_id_string,
                    "created_at": "2024-01-15T10:30:00Z"
                }]));
        });
    ...
    #[tokio::test]
    async fn find_mint_by_issuer_request_id_propagates_api_errors() {
        let server = MockServer::start();
        let client = create_test_client(&server);
        let issuer_id = issuer_request_id("failed-lookup");
        let lookup_mock = server.mock(|when, then| {
            when.method(GET)
                .path(tokenization_requests_path())
                .query_param("type", "mint");
    ...
    #[tokio::test]
    async fn find_mint_by_issuer_request_id_rejects_duplicate_history() {
        let server = MockServer::start();
        let client = create_test_client(&server);
        ...
        let error = client
            .find_mint_by_issuer_request_id(&issuer_id)
            .await
            .expect_err("duplicate mint history must fail reconciliation");

        assert!(matches!(
            error,
            AlpacaTokenizationError::DuplicateMintIssuerRequestId {
                issuer_request_id
            } if issuer_request_id == issuer_id
        ));
        lookup_mock.assert();
    }
    ...
    async fn get_request_refuses_an_entry_that_omits_network() {
        ...
        let error = service
            .get_request(&tokenization_request_id("req_1"))
            .await
            .unwrap_err();
        ...
        assert!(
            matches!(
                error,
                AlpacaTokenizationError::NetworkMissing { ref id }
