...
use alloy_primitives::{Address, TxHash, hex::FromHexError};
...
impl AlpacaWalletClient {
    ...
    /// # Errors
    ///
    /// Returns [`AlpacaWalletError::PrivateKeyJwtUnsupported`] for
    /// `PrivateKeyJwt` credentials, [`AlpacaWalletError::InvalidBaseUrl`] for
    /// a base URL that is not HTTPS (or HTTP on a loopback host), or the HTTP
    /// client or authentication runtime error.
    pub fn new(
        base_url: String,
        account_id: AlpacaAccountId,
        auth: AlpacaAuth,
...
mod tests {
    ...
    #[test]
    fn new_rejects_a_non_loopback_http_base_url() {
        let Err(error) = AlpacaWalletClient::new(
            "http://broker-api.sandbox.alpaca.markets".to_string(),
            TEST_ACCOUNT_ID,
            AlpacaAuth::Basic {
                api_key: "test_key_id".to_string(),
                api_secret: "test_secret_key".to_string(),
        ...
        assert!(
            matches!(
                error,
                AlpacaWalletError::InvalidBaseUrl(EndpointError::InsecureScheme { .. })
            ),
            "{error:?}"
        );
    }
    ...
    async fn credentialed_requests_do_not_follow_redirects() {
        ...
        let error = client.get("/redirect").await.unwrap_err();

        assert!(
            matches!(
                error,
                AlpacaWalletError::ApiError { status, .. } if status == StatusCode::FOUND
            ),
            "{error:?}"
        );
        redirect.assert();
