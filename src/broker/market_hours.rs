...
use crate::broker::{MarketSession, MarketSessionStatus, PostCloseGap};
...
async fn session_and_close_at(
    ...
    log_session_bound_drift(&today_calendar);
...
/// Detects a silent redefinition of the undocumented session bounds (see the
/// CONTRACT RISK note on `CalendarDay`). A NARROWED window is the dangerous
/// direction -- fills landing inside the assumed 04:00-20:00 extended window
/// but outside Alpaca's would classify Closed and sit unhedged with no broker
/// rejection to surface it -- so warn loudly when the broker's bounds differ
/// from the documented extended-hours window.
fn log_session_bound_drift(calendar_day: &CalendarDay) {
    let expected_session_open = NaiveTime::from_hms_opt(4, 0, 0);
    let expected_session_close = NaiveTime::from_hms_opt(20, 0, 0);
    if Some(calendar_day.session_open) != expected_session_open {
        warn!(
            session_open = %calendar_day.session_open,
    ...
    if Some(calendar_day.session_close) != expected_session_close {
        debug!(
            session_close = %calendar_day.session_close,
...
mod tests {
    ...
    #[tokio::test]
    async fn weeknight_overnight_has_no_extended_close_metadata_like_the_sunday_leg() {
        // A weeknight Overnight evening takes the today-calendar branch, but
        // its enum variant must not expose today's already-past 20:00 close.
        let server = MockServer::start();
        let ctx = create_test_ctx(AlpacaBrokerApiMode::Mock(server.base_url()));
        mock_trading_day(&server, "2025-01-06");
        mock_next_trading_day(&server, "2025-01-07", "2025-01-20", Some("2025-01-07"));
