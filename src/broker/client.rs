...
impl AlpacaBrokerApiClient {
    pub(crate) fn new(ctx: &AlpacaBrokerApiCtx) -> Result<Self, AlpacaBrokerApiError> {
        ...
        // a host the configured mode did not name.
        ...
        validate_origin(ctx.base_url(), EndpointRole::BaseUrl)?;
        validate_origin(ctx.mode().market_data_base_url(), EndpointRole::BaseUrl)?;
    ...
    pub(crate) async fn market_data_get(
        &self,
        path: &str,
    ) -> Result<reqwest::RequestBuilder, crate::auth::KmsJwtError> {
...
mod tests {
    ...
    use super::*;
    use crate::broker::auth::AlpacaAccountId;
    use crate::broker::{AssetStatus, TimeInForce};
    use crate::broker::{FractionalShares, Positive};
    ...
    #[test]
    fn client_rejects_a_non_loopback_http_base_url() {
        let ctx = create_test_ctx(AlpacaBrokerApiMode::Mock(
            "http://broker.example".to_string(),
        ));

        let Err(error) = AlpacaBrokerApiClient::new(&ctx) else {
            panic!("a cleartext non-loopback base URL must be rejected");
        };

        assert!(
            matches!(
                error,
                AlpacaBrokerApiError::InvalidEndpoint(
                    crate::endpoint::EndpointError::InsecureScheme { .. }
                )
            ),
            "{error:?}"
        );
    ...
    #[tokio::test]
    async fn credentialed_requests_do_not_follow_redirects() {
    ...
    async fn test_create_journal_success() {
        ...
        assert_eq!(response.status, crate::broker::JournalStatus::Pending);
