//! Broker API surface used by st0x.liquidity: account verification, assets,
//! equity market and limit orders, USD/USDC conversion orders, positions and
//! account funds, security journals, account activities, calendar and clock
//! market hours, and Market Data API quotes.
//!
//! [`AlpacaBrokerApi`] is the entry point. It owns the authenticated client,
//! the asset cache, and the configured time-in-force; the consumer's executor
//! abstraction wraps it.
...
pub use amount::AlpacaAmount;
// `AssetDetails` is the CLI's overnight/fractional asset-inspection surface.
pub use api::{AlpacaBrokerApi, AssetDetails, PreparedShares};
...
pub use lifecycle::{
    CancellationOutcome, ClientOrderId, ClientOrderIdError, Direction, ExecutorOrderId,
    InvalidDirectionError, LimitOrder, MarketOrder, OrderFailureTerminality, OrderPlacement,
    OrderState, OrderStatus, OrderUpdate, ParseOrderStatusError, RecoveredOrderPlacement,
};
...
/// Stateful httpmock Alpaca Broker API server for consumer e2e suites.
///
/// Lives in its own module because its wire-level `OrderStatus` and the
/// wallet `TransferStatus` / `WhitelistStatus` would clash with the broker
/// and wallet domain types of the same names.
#[cfg(feature = "mock")]
pub mod mock {
    pub use super::mock_api::{
...
pub enum AlpacaBrokerApiError {
    ...
    #[error(transparent)]
    InvalidEndpoint(#[from] crate::endpoint::EndpointError),
    ...
    InvalidSymbol(#[from] crate::broker::EmptySymbolError),
