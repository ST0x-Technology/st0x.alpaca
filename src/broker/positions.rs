...
pub struct Inventory {
    ...
    /// Always `Some` from this client; `Some(Usdc::ZERO)` for a zero balance
    /// so the snapshot is still emitted.
    ...
    /// margin. `None` when the broker omits the field or the value cannot be
    /// converted.
    ...
    /// proceeds. `None` when the broker omits the field.
