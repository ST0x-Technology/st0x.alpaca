//! Broker order lifecycle types: idempotency keys, order requests, placement
//! and recovery results, status snapshots, and cancellation outcomes.
...
///
/// Serializes as `snake_case` (`"buy"`/`"sell"`). Deserialization and
/// [`FromStr`] accept either case (`"buy"`, `"Buy"`, `"BUY"`), so legacy
/// persisted spellings keep loading. [`Display`] renders `BUY`/`SELL`.
    ...
    fn from_str(value: &str) -> Result<Self, Self::Err> {
        match value.to_ascii_uppercase().as_str() {
            ...
            _ => Err(InvalidDirectionError {
                direction_provided: value.to_string(),
            }),
        }
    }
}
...
/// The order id the broker assigned when an order was placed.
...
mod tests {
    ...
    #[test]
    fn direction_serializes_snake_case_and_accepts_legacy_spellings() {
        assert_eq!(serde_json::to_value(Direction::Buy).unwrap(), json!("buy"));
        assert_eq!(
            serde_json::to_value(Direction::Sell).unwrap(),
            json!("sell")
        );
        for (wire, expected) in [
            ...
            assert_eq!(
                serde_json::from_value::<Direction>(json!(wire)).unwrap(),
                expected
            );
        }
