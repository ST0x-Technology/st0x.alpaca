...
use super::{Symbol, format_float_with_fallback};
...
impl AlpacaBrokerMock {
    ...
    /// # Panics
    ///
    /// Panics when no transfer with `transfer_id` exists.
    ...
    /// # Errors
    ///
    /// Returns the [`FloatError`] from the quantity or market-value
    /// arithmetic.
    pub fn adjust_position(&self, symbol: &Symbol, delta: Float) -> Result<(), FloatError> {
        let mut state = lock(&self.state);
        ...
        if let Some(position) = state.account.positions.get_mut(symbol) {
            ...
            let market_value = price.map_or(Ok(float!(0)), |price| delta * price)?;
    ...
    /// # Errors
    ///
    /// Returns the provider error when the starting block number cannot be
    /// read.
...
fn add_wallet_balance(state: &mut MockState, asset: &str, amount: Float) -> Result<(), FloatError> {
    let current = state
        .wallet_balances
        .get(asset)
        .copied()
...
fn register_positions_endpoint(server: &MockServer, state: &Arc<Mutex<MockState>>) {
    ...
    server.mock(move |when, then| {
        ...
        then.respond_with(move |request: &HttpMockRequest| {
            let result = (|| -> Result<Vec<Value>, FloatError> {
                let state = lock(&state);
                let mut positions = state
                    ...
                    .values()
                    .map(|pos| {
                        let is_neg = pos.quantity.lt(Float::from_raw(B256::ZERO))?;
    ...
    server.mock(|when, then| {
        ...
        then.respond_with(move |request: &HttpMockRequest| {
            let OrderRequest {
                ...
                side,
                client_order_id,
            } = match parse_order_request(request) {
                Ok(parsed) => parsed,
...
/// A validated order-placement request body.
struct OrderRequest {
    symbol: Symbol,
    sizing: OrderSizing,
    side: OrderSide,
...
/// Validates an order-placement body, answering 400 on the first invalid
/// field in the order the real endpoint checks them.
fn parse_order_request(request: &HttpMockRequest) -> Result<OrderRequest, HttpMockResponse> {
    ...
    let Ok(symbol) = Symbol::new(symbol) else {
        return Err(json_response(
            400,
            &json!({"message": "symbol cannot be empty"}),
        ));
    };
...
fn handle_crypto_order(
    ...
    let quantized_quantity = match quantize_crypto_sizing(sizing) {
        Ok(quantized) => quantized,
        Err(response) => return response,
    };

    if let Err(response) = apply_crypto_balances(state, side, quantized_quantity) {
        return response;
    }
...
/// Truncates a crypto order's size to the precision the real API accepts
/// (six decimals for a USDC quantity, whole cents for a notional), answering
/// 422 when the requested size is below or exceeds that precision.
fn quantize_crypto_sizing(sizing: OrderSizing) -> Result<Float, HttpMockResponse> {
    ...
    let quantized_quantity = match super::truncate_to_decimal_places(quantity, max_decimal_places) {
        Ok(Some(value)) => value,
        Ok(None) => {
            return Err(json_response(
                422,
                &json!({"message": below_precision_message}),
            ));
        }
        Err(error) => {
            return Err(json_response(
    ...
    if !matches_requested_precision {
        return Err(json_response(
            422,
            &json!({"message": excess_precision_message}),
        ));
    }

    Ok(quantized_quantity)
}

/// Debits/credits USD cash and the USDC wallet balance for a filled crypto
/// order, answering 422 when a sell exceeds the USDC balance. Leaves state
/// untouched on any error.
fn apply_crypto_balances(
    state: &mut MockState,
    side: OrderSide,
    quantized_quantity: Float,
) -> Result<(), HttpMockResponse> {
        ...
        OrderSide::Buy => {
            ...
            let position = state
                ...
                .or_insert_with(|| MockPosition {
                    symbol: symbol_key,
                    quantity: Float::from_raw(B256::ZERO),
                    market_value: Float::from_raw(B256::ZERO),
                });
...
fn register_wallet_transfers_post_endpoint(server: &MockServer, state: &Arc<Mutex<MockState>>) {
    ...
    server.mock(|when, then| {
        ...
        then.respond_with(move |request: &HttpMockRequest| {
            let (amount, asset, to_address) = match parse_wallet_transfer_request(request) {
                Ok(parsed) => parsed,
                Err(response) => return response,
            };
...
/// Validates a wallet-transfer body, answering 400 on the first missing or
/// invalid field.
fn parse_wallet_transfer_request(
    request: &HttpMockRequest,
) -> Result<(Float, String, Address), HttpMockResponse> {
    let body: Value = match serde_json::from_slice(request.body().as_ref()) {
        Ok(parsed) => parsed,
        Err(parse_error) => {
            return Err(json_response(
    ...
    Ok((amount, asset.to_string(), to_address))
}
...
fn format_u256_as_usdc(raw: U256) -> String {
...
mod tests {
    ...
    use crate::broker::client::AlpacaBrokerApiClient;
    use crate::broker::order::{ConversionPollDeadlines, poll_crypto_order_until_filled};
    use crate::broker::{
        AccountActivitiesQuery, AlpacaAccountId, AlpacaBrokerApi, AlpacaBrokerApiCtx,
        AlpacaBrokerApiError, AlpacaBrokerApiMode, AlpacaMarketDataError, AssetDetails,
        AssetStatus, ClientOrderId, ConversionOrder, Direction, MarketOrder, MarketSession,
        TimeInForce, market_hours,
    };
    use crate::broker::{FractionalShares, Positive, Symbol, Usd, Usdc};
    use crate::core::{AlpacaAuth, Permanence};
    ...
    async fn overnight_knobs_round_trip_through_the_real_parsers() {
        ...
        assert!(
            matches!(
                source.as_ref(),
                AlpacaMarketDataError::MissingQuote { symbol }
        ...
        let forbidden = executor
            ...
            .await
            .unwrap_err();
        assert_eq!(forbidden.permanence(), Permanence::Permanent);
    ...
    /// Baseline state for the crypto-order handler tests: $100 cash and
    /// buying power, happy-path mode, and the given USDC wallet balances.
    fn crypto_order_test_state(wallet_balances: HashMap<String, Float>) -> MockState {
        MockState {
            account: MockAccount {
                cash: float!(100),
    ...
    /// Broker-API context pointed at the given mock, for driving a client
    /// or executor against it.
    fn mock_broker_ctx(mock: &AlpacaBrokerMock) -> AlpacaBrokerApiCtx {
        AlpacaBrokerApiCtx {
            auth: AlpacaAuth::Basic {
                api_key: TEST_API_KEY.to_string(),
                api_secret: TEST_API_SECRET.to_string(),
    ...
    async fn delayed_sip_quote_is_served_by_the_mock_broker() {
        ...
        let quote = executor.fetch_latest_quote(&symbol).await.unwrap();
    ...
    async fn asset_endpoint_marks_default_mock_assets_fractionable() {
        ...
        let mock = AlpacaBrokerMock::start()
            ...
            .call()
            .await;
        let client = AlpacaBrokerApiClient::new(&mock_broker_ctx(&mock)).unwrap();
    ...
    async fn fill_records_a_fee_activity() {
        ...
        let activities = mock_broker_ctx(&mock)
            .fetch_account_activities(&AccountActivitiesQuery::pnl(None, None))
            .await
            .unwrap();
    ...
    fn crypto_cash_delta_is_quantized_to_cents() {
        let mut state = crypto_order_test_state(HashMap::new());
    ...
    fn crypto_sell_debits_wallet_usdc_before_crediting_cash() {
        let mut state = crypto_order_test_state(HashMap::from([("USDC".to_string(), float!(20))]));
    ...
    async fn notional_usd_to_usdc_buy_round_trips_against_the_mock() {
        ...
        let placed = crate::broker::order::convert_usdc_usd(
            &client,
            ConversionOrder::BuyWithUsd(Positive::new(Usd::new(float!(500.25))).unwrap()),
        ...
        let settled =
            poll_crypto_order_until_filled(&client, placed.id, ConversionPollDeadlines::PRODUCTION)
                .await
                .unwrap();
