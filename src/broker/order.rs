...
impl AlpacaLimitPrice {
    /// Validates Alpaca's limit-price precision.
    ///
    /// # Errors
    ///
    /// Returns [`AlpacaBrokerApiError::InvalidLimitPricePrecision`] when the
    /// price has more decimals than Alpaca accepts for its range.
...
impl CryptoOrderResponse {
    ...
    pub fn status_display(&self) -> &'static str {
        use BrokerOrderStatus::{
            Accepted, AcceptedForBidding, Calculated, Canceled, DoneForDay, Expired, Filled, New,
            PartiallyFilled, PendingCancel, PendingNew, PendingReplace, Rejected, Replaced,
            Stopped, Suspended,
        };
...
fn classify_broker_status(
    ...
    use BrokerOrderStatus::{
        Accepted, AcceptedForBidding, Calculated, Canceled, DoneForDay, Expired, Filled, New,
        PartiallyFilled, PendingCancel, PendingNew, PendingReplace, Rejected, Replaced, Stopped,
        Suspended,
    };
...
fn truncate_shares_to_alpaca_precision(
    ...
    let truncated_float = crate::broker::truncate_to_decimal_places(
        original,
        crate::broker::ALPACA_MAX_DECIMAL_PLACES,
    )?
    .ok_or(AlpacaBrokerApiError::BelowPrecision {
        shares,
        max_decimals: crate::broker::ALPACA_MAX_DECIMAL_PLACES,
    })?;
...
fn validate_usdc_amount_for_alpaca_precision(amount: Float) -> Result<Float, AlpacaBrokerApiError> {
    let truncated_amount =
        crate::broker::truncate_to_decimal_places(amount, ALPACA_CRYPTO_MAX_DECIMAL_PLACES)?
...
fn truncate_notional_to_whole_cents(amount: Float) -> Result<Float, AlpacaBrokerApiError> {
    let truncated =
        crate::broker::truncate_to_decimal_places(amount, ALPACA_NOTIONAL_MAX_DECIMAL_PLACES)?
            .ok_or(AlpacaBrokerApiError::UsdcBelowPrecision {
                amount,
...
mod tests {
    ...
    fn kms_jwt_errors_are_never_the_duplicate_order_case() {
        let error = AlpacaBrokerApiError::KmsJwt(crate::broker::KmsJwtError::ClockBeforeEpoch);
    ...
    fn classify_maps_every_broker_status_to_its_outcome() {
        use crate::broker::CryptoOrderFailureReason;
        ...
        assert!(
            matches!(
                ...
                AlpacaBrokerApiError::BelowPrecision {
                    max_decimals,
                    ..
                } if max_decimals == crate::broker::ALPACA_MAX_DECIMAL_PLACES
            ),
